...
class UltimateInventoryAppV3:
    ...
    def init_csv(self):
        ...
        if not os.path.exists(FILE_NAME):
            ...
            with open(FILE_NAME, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["タブ名", "品名", "在庫数", "単位", "更新日時", "保管場所", "備考"])
                writer.writerow(["事務用品", "A4コピー用紙", 12, "箱", now, "キャビネットA", "来月発注予定"])
                writer.writerow(["事務用品", "黒ボールペン 0.5mm", 2, "本", now, "引き出し1", ""])
                writer.writerow(["PC・機材", "HDMIケーブル", 3, "本", now, "機材庫", "2mタイプ"])
                writer.writerow(["その他", "", "", "", "", "", ""])
    ...
    def create_context_menu(self):
        ...
        self.context_menu.add_command(label="＋ 行の挿入 (新規追加)", command=self.add_item_dialog)
        self.context_menu.add_command(label="🗑️ 選択行の削除", command=self.delete_item)
        self.context_menu.add_separator()
        self.context_menu.add_command(label="📊 エクセル向け出力", command=self.export_excel)
        self.context_menu.add_command(label="📄 PDF向け出力", command=self.export_pdf)
    ...
    def create_widgets(self):
        ...
        tk.Button(
            header_btn_frame, text="＋ タブ追加", font=("MeiryoUI", 9, "bold"),
            bg="#27AE60", fg="white", bd=0, padx=10, pady=4, cursor="hand2", command=self.add_tab
        ).pack(side="left", padx=3)

        tk.Button(
            header_btn_frame, text="🗑️ タブ削除", font=("MeiryoUI", 9),
            bg="#E74C3C", fg="white", bd=0, padx=10, pady=4, cursor="hand2", command=self.delete_tab
        ).pack(side="left", padx=3)
        ...
        card_frame = tk.Frame(main_frame, bg="white", relief="solid", bd=1, padx=15, pady=10)
        card_frame.pack(fill="x", pady=(0, 10))

        # 全タブ横断検索
        search_frame = tk.Frame(card_frame, bg="white")
        search_frame.pack(fill="x", pady=(0, 8))
        tk.Label(search_frame, text="🔍 全タブ横断検索:", font=("MeiryoUI", 9, "bold"), bg="white").pack(side="left", padx=(0, 5))
        self.search_var = tk.StringVar()
        self.search_var.trace_add("write", lambda *args: self.filter_data())
        search_entry = tk.Entry(search_frame, textvariable=self.search_var, font=("MeiryoUI", 10), bg="#F0F3F4", bd=1, relief="flat")
        ...
        # 1段目
        tk.Label(input_frame, text="品名:", font=("MeiryoUI", 9, "bold"), bg="white").grid(row=0, column=0, sticky="w")
        self.entry_name = tk.Entry(input_frame, width=20, font=("MeiryoUI", 9), bg="#F0F3F4", bd=1, relief="flat")
        self.entry_name.grid(row=0, column=1, padx=(5, 10), ipady=3)

        tk.Label(input_frame, text="数量:", font=("MeiryoUI", 9, "bold"), bg="white").grid(row=0, column=2, sticky="w")
        self.entry_qty = tk.Entry(input_frame, width=6, font=("MeiryoUI", 9), bg="#F0F3F4", bd=1, relief="flat")
        self.entry_qty.grid(row=0, column=3, padx=(5, 10), ipady=3)

        tk.Label(input_frame, text="単位:", font=("MeiryoUI", 9, "bold"), bg="white").grid(row=0, column=4, sticky="w")
        self.combo_unit = ttk.Combobox(
            input_frame, values=["個", "本", "枚", "箱", "セット", "台", "kg", "L", "冊", "パック"],
            width=6, font=("MeiryoUI", 9)
        )
        self.combo_unit.set("個")
        self.combo_unit.grid(row=0, column=5, padx=(5, 15), ipady=3)

        # 2段目
        tk.Label(input_frame, text="保管場所:", font=("MeiryoUI", 9, "bold"), bg="white").grid(row=1, column=0, sticky="w", pady=(8,0))
        self.entry_loc = tk.Entry(input_frame, width=20, font=("MeiryoUI", 9), bg="#F0F3F4", bd=1, relief="flat")
        self.entry_loc.grid(row=1, column=1, padx=(5, 10), pady=(8,0), ipady=3)

        tk.Label(input_frame, text="備考:", font=("MeiryoUI", 9, "bold"), bg="white").grid(row=1, column=2, sticky="w", pady=(8,0))
        self.entry_rem = tk.Entry(input_frame, width=30, font=("MeiryoUI", 9), bg="#F0F3F4", bd=1, relief="flat")
        self.entry_rem.grid(row=1, column=3, columnspan=3, sticky="we", padx=(5, 15), pady=(8,0), ipady=3)

        # 登録ボタン
        btn_add = tk.Button(
            input_frame, text="＋ 登録", font=("MeiryoUI", 9, "bold"),
            bg=ACCENT_BLUE, fg="white", bd=0, padx=20, pady=10, cursor="hand2", command=self.add_item
        )
        ...
        tk.Button(
            action_frame, text="➖ 1つ使う", font=("MeiryoUI", 9, "bold"), bg=ACCENT_ORANGE, fg="white",
            bd=0, padx=10, pady=6, cursor="hand2", command=self.use_item
        ).pack(side="left", padx=(0, 5))
        
        tk.Button(
            action_frame, text="➕ 1つ補充", font=("MeiryoUI", 9, "bold"), bg=ACCENT_GREEN, fg="white",
            bd=0, padx=10, pady=6, cursor="hand2", command=self.add_one_item
        ).pack(side="left", padx=5)

        tk.Label(action_frame, text="※ヘッダー(列名)クリックで並び替え / 右クリックで各種メニュー表示", font=("MeiryoUI", 8), bg=BG_COLOR, fg="#7F8C8D").pack(side="left", padx=15)

        tk.Button(
            action_frame, text="🔄 最新表示", font=("MeiryoUI", 9), bg="#95A5A6", fg="white",
            bd=0, padx=10, pady=6, cursor="hand2", command=self.load_data
        ).pack(side="right")
    ...
    def rebuild_notebook_tabs(self, items):
        ...
        for tab_name in items.keys():
            ...
            # 各列の設定とソート機能のバインド
            for col in columns:
                tree.heading(col, text=col, command=lambda _t=tree, _c=col: self.treeview_sort_column(_t, _c, False))
            ...
            tree.tag_configure("empty", background="#FADBD8", foreground="#78281F")
            tree.tag_configure("low", background="#FCF3CF", foreground="#7D6608")
    ...
    def treeview_sort_column(self, tree, col, reverse):
        ...
        # 次回クリック時は逆順にする設定
        tree.heading(col, command=lambda _t=tree, _c=col: self.treeview_sort_column(_t, _c, not reverse))
                ...
                for row in reader:
                    if len(row) >= 1 and row[0].strip():
                        ...
                        if len(row) >= 3 and row[1].strip():
                            ...
                            items[tab][name] = {
                                "qty": qty, "unit": unit,
                                "updated_at": updated_at, "location": loc, "remarks": rem
                            }
            ...
            for tab, tab_items in items.items():
                if not tab_items:
                    ...
                    for name, data in tab_items.items():
                        writer.writerow([
                            tab, name, data["qty"], data["unit"],
                            data.get("updated_at", ""), data.get("location", ""), data.get("remarks", "")
                        ])
    ...
    def load_data(self, filter_text=""):
        ...
        for tab_name, tree in self.trees.items():
            ...
            for name, data in tab_items.items():
                ...
                tree.insert(
                    "", "end", values=(name, qty, unit, updated, loc, rem, status), tags=(tag,)
                )
    ...
    def add_tab(self):
        ...
        if not new_tab or not new_tab.strip(): return
    ...
    def on_notebook_double_click(self, event):
        try: index = self.notebook.index(f"@{event.x},{event.y}")
        except: return
        items = self.get_all_items()
        old_tab = list(items.keys())[index]
        new_tab = simpledialog.askstring("名前変更", "新しいタブ名:", initialvalue=old_tab)
        if not new_tab or new_tab.strip() == old_tab: return
        new_tab = new_tab.strip()
        if new_tab in items: return
        new_items = {new_tab if k == old_tab else k: v for k, v in items.items()}
    ...
    def on_row_double_click(self, event):
        ...
        old_name, old_qty, old_unit = str(values[0]), int(values[1]), str(values[2])
        old_loc, old_rem = str(values[4]), str(values[5])
        items = self.get_all_items()
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")

        if column == "#1":  # 品名
            new_name = simpledialog.askstring("編集", "新しい品名:", initialvalue=old_name)
            ...
            new_qty = simpledialog.askinteger("編集", "正しい在庫数:", initialvalue=old_qty, minvalue=0)
            ...
            new_unit = simpledialog.askstring("編集", "新しい単位:", initialvalue=old_unit)
            ...
            new_loc = simpledialog.askstring("編集", "保管場所:", initialvalue=old_loc)
            ...
            new_rem = simpledialog.askstring("編集", "備考:", initialvalue=old_rem)
    ...
    def add_item(self):
        ...
        if not name or not qty.isdigit():
            messagebox.showwarning("エラー", "品名と正しい数字を入力してください。")
            return

        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        items = self.get_all_items()
        items[tab_name][name] = {"qty": int(qty), "unit": unit, "updated_at": now, "location": loc, "remarks": rem}
        self.save_all_items(items)
    ...
    def add_item_dialog(self):
        ...
        tab_name, _ = self.get_current_tab_info()
        new_name = simpledialog.askstring("挿入", f"「{tab_name}」に追加する品名を入力:")
        ...
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        items[tab_name][new_name] = {"qty": 0, "unit": "個", "updated_at": now, "location": "", "remarks": ""}
        self.save_all_items(items)
        self.load_data(self.search_var.get())
        messagebox.showinfo("追加完了", f"「{new_name}」を挿入しました。\n一覧から行をダブルクリックして個数等を編集してください。")
    ...
    def export_excel(self):
        ...
        try:
            with open(file_path, "w", newline="", encoding="shift-jis", errors="replace") as f:
                writer = csv.writer(f)
                writer.writerow(["品名", "在庫数", "単位", "更新日時", "保管場所", "備考", "状態"])
                for row in tree.get_children():
                    writer.writerow(tree.item(row)["values"])
            messagebox.showinfo("完了", f"エクセル向けに出力しました。\n保存先: {file_path}")
